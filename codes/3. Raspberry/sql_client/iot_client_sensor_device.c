...
//		name_msg[str_len-1] = 0;   //'\n' 제거
		...
		motor = atof(pArray[1]);
		distance = atof(pArray[2]);
		speed = atof(pArray[3]);
  		sprintf(sql_cmd, "insert into car_table(car_speed, car_distance, car_motor, date, time) values(%f, %f, %f, now(), now())",speed, distance, motor);
